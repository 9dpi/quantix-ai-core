...
for i, s in enumerate(r.json()):
    ...
    if state in ['TP_HIT', 'SL_HIT', 'CANCELLED'] and not s.get('closed_at'):
        issues.append("[FAIL] Terminal state without closed_at!")
    
    # Check 2: ENTRY_HIT must have entry_hit_at
    if state == 'ENTRY_HIT' and not s.get('entry_hit_at'):
        issues.append("[FAIL] ENTRY_HIT without entry_hit_at!")
    
    # Check 3: TP_HIT should have result=PROFIT
    if state == 'TP_HIT' and result != 'PROFIT':
        issues.append(f"[FAIL] TP_HIT but result={result} (expected PROFIT)")
    
    # Check 4: SL_HIT should have result=LOSS
    if state == 'SL_HIT' and result != 'LOSS':
        issues.append(f"[FAIL] SL_HIT but result={result} (expected LOSS)")
    
    # Check 5: CANCELLED should have result=CANCELLED or EXPIRED
    if state == 'CANCELLED' and result not in ['CANCELLED', 'EXPIRED', 'CLOSED_TIMEOUT', 'SL_INVALIDATED']:
        issues.append(f"[WARN] CANCELLED but result={result}")
    
    # Check 6: Telegram ID should exist for published signals
    if not s.get('telegram_message_id'):
        issues.append("[WARN] No Telegram message ID")
    ...
    sl = s['sl']
    if s['direction'] == 'BUY':
        if tp <= entry: issues.append(f"[FAIL] BUY but TP({tp}) <= Entry({entry})")
        if sl >= entry: issues.append(f"[FAIL] BUY but SL({sl}) >= Entry({entry})")
    else:
        if tp >= entry: issues.append(f"[FAIL] SELL but TP({tp}) >= Entry({entry})")
        if sl <= entry: issues.append(f"[FAIL] SELL but SL({sl}) >= Entry({entry})")
