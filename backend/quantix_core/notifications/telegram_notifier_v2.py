...
class TelegramNotifierV2:
    ...
    def send_waiting_for_entry(self, signal: dict) -> Optional[int]:
        ...
        message = (
            ...
            f"Entry Price: {entry}\n"
            f"Take Profit: {tp} ({signal.get('tp_pips', 7.0)} pips)\n"
            f"Stop Loss: {sl} ({signal.get('sl_pips', 12.0)} pips)\n\n"
            f"Lot size: Dynamic (1% Risk Ratio)\n"
    ...
    def send_entry_hit(self, signal: dict) -> Optional[int]:
        ...
        message = (
            ...
            f"🎯 TP: {signal.get('tp')} ({signal.get('tp_pips', 7.0)} pips)\n"
            f"🛑 SL: {signal.get('sl')} ({signal.get('sl_pips', 12.0)} pips)\n\n"
    ...
    def send_tp_hit(self, signal: dict) -> Optional[int]:
        ...
        message = (
            ...
            f"TP: {signal.get('tp')}\n\n"
            f"Status: 🏁 CLOSED\_TP\n"
            f"Result: 🟢 PROFIT (+{signal.get('tp_pips', 7.0)} pips)\n"
    ...
    def send_sl_hit(self, signal: dict) -> Optional[int]:
        ...
        message = (
            ...
            f"SL: {signal.get('sl')}\n\n"
            f"Status: 🏁 CLOSED\_SL\n"
            f"Result: 🔴 LOSS (-{signal.get('sl_pips', 12.0)} pips)\n"
            f"R:R: 1 : {signal.get('reward_risk_ratio', 1.0)}\n\n"
            f"Signal lifecycle completed."
        )
