...
for attempt in range(1, MAX_RESTARTS + 1):
    ...
    log_to_db("SYSTEM_WATCHDOG", f"STARTING_ATTEMPT_{attempt}", "LAUNCHER")
    ...
    try:
        ...
        def log_stream(stream):
            for line in iter(stream.readline, ''):
                ...
                if clean_line:
                    print(f"[WATCHDOG] {clean_line}")
                    log_to_db("WATCHDOG_LOG", clean_line[:200], "STDOUT")
                    
        t = threading.Thread(target=log_stream, args=(process.stdout,), daemon=True)
        t.start()
        
        exit_code = process.wait()
        print(f"⚠️ Watchdog exited with code {exit_code}")
        log_to_db("SYSTEM_WATCHDOG", f"EXITED_CODE_{exit_code}", "LAUNCHER")
        ...
        log_to_db("SYSTEM_WATCHDOG", f"ERROR_{str(e)[:50]}", "LAUNCHER")
